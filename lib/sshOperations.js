...
const fileCert2String = (argFile) => {
    try {
        //
        let outStrCert = fs.readFileSync(argFile.pathPrivateKey,'utf-8').trim() ;
        //
        return outStrCert ;
        //
    } catch(errGSC){
...
const sftpFastPut = (argSftp,argFile,argOpt) => {
    return new Promise(function(respOk,respRech){
        try {
            ...
            console.log('.....EXISTE:: ',( fs.existsSync(argFile.localFullFilePath) ),';') ;
            //
        ...
        try {
            ...
            argSSHconn.exec(argFile[argCmd], function(errCmd,stream) {
                if (errCmd) {
                    ...
                    stream.on('exit',function(){ /* console.log('....no exit') ; */ });
        ...
        return new Promise(function(respOk,respRech){
            try {
                ...
                for ( let posFF=0; posFF<argArrayFiles.length; posFF++ ){
                    ...
                    fileElem.command            = ` cp -vvv -W "seqparms='${fileElem.dsnDcb}'" ${fileElem.remoteFullFilePath} "//'${fileElem.remoteDataset}'" `
